...
<section class="content-section">
    <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
        ...
        <div class="timeline-content" style="max-width: 100%;">
            ...
            ?>
                <div style="margin-top: 1.5rem; padding-top: 1.5rem; border-top: 1px solid var(--border-color);">
                    ...
                    <?php if ($related_memory) : ?>
                        <p><strong>相关相册：</strong><a href="<?php echo get_permalink($related_memory); ?>"><?php echo get_the_title($related_memory); ?></a></p>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
            ...
            <?php if ($post_type === 'memory') :
                $memory_date = get_post_meta(get_the_ID(), '_memory_date', true);
                $memory_location = get_post_meta(get_the_ID(), '_memory_location', true);
                $related_moment = get_post_meta(get_the_ID(), '_related_moment', true);
                $photos = brave_get_memory_photos(get_the_ID());
            ?>
                <div style="margin-top: 1.5rem; padding-top: 1.5rem; border-top: 1px solid var(--border-color);">
                    <?php if ($memory_date) : ?>
                        <p style="margin-bottom: 0.5rem;"><strong>拍摄日期：</strong><?php echo esc_html($memory_date); ?></p>
                    <?php endif; ?>
                    <?php if ($memory_location) : ?>
                        <p style="margin-bottom: 0.5rem;"><strong>地点：</strong><?php echo esc_html($memory_location); ?></p>
                    <?php endif; ?>
                    <?php if ($related_moment) : ?>
                        <p style="margin-bottom: 0.5rem;"><strong>相关点滴：</strong><a href="<?php echo get_permalink($related_moment); ?>"><?php echo get_the_title($related_moment); ?></a></p>
                    <?php endif; ?>
                    
                    <?php if (!empty($photos)) : ?>
                        <div style="margin-top: 1rem;">
                            <p style="margin-bottom: 0.5rem;"><strong>照片（<?php echo count($photos); ?>张）：</strong></p>
                            <div class="memory-grid" style="grid-template-columns: repeat(auto-fill, minmax(120px, 1fr));">
                                <?php foreach ($photos as $photo) : ?>
                                    <img src="<?php echo esc_url($photo['thumb']); ?>" alt="" class="memory-cover" style="cursor: pointer;" onclick="window.open('<?php echo esc_url($photo['url']); ?>', '_blank')">
                                <?php endforeach; ?>
                            </div>
                        </div>
                    <?php endif; ?>
